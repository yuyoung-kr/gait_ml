...
def load_metadata():
    ...
    required_cols = ['subject_id', 'condition', 'trial_id']
...
def get_metadata(filename: str):
    ...
    return row['subject_id'], row['condition'], row['trial_id'], row['activity_label']
...
def annotate_file_labels(df, condition):
    df = df.copy()
    ...
    e1_idx = df.index[df['event_code'] == 1].tolist()
    e2_idx = df.index[df['event_code'] == 2].tolist()

    first_e1 = e1_idx[0] if e1_idx else None
    first_e2 = e2_idx[0] if e2_idx else None

    if first_e1 is not None:
        df.loc[first_e1:, 'activity_label'] = 'walking'
    else:
        df['activity_label'] = 'walking'

    if first_e2 is not None:
        df.loc[first_e2:, 'steady_mask'] = 1

    tail_n = int(TAIL_TRIM_S * TARGET_FS)
    if tail_n > 0 and tail_n < len(df):
        df.loc[df.index[-tail_n:], 'steady_mask'] = 0
...
def process_one_raw(raw_path: Path):
    subject_id, condition, trial_id, _ = get_metadata(raw_path.name)
    ...
    df_aligned = annotate_file_labels(df_aligned, condition)
    ...
    print(f"   → 生成 segments: {len(segments_list)} 个")
